...
//HUBUNGI 6281359053674 JIKA ADA ORANG YANG JUAL SCRIPT INIðŸ’¢ðŸ’­
...
isForwarded: true, 
	...
	body: `${ucapanWaktu} ngaf ${pushname}`,
	previewType:"PHOTO",
	//caption: `oke Tunggu Bentar ngaf`,
...
body: `Hai  ${ucapanWaktu} ngaf ${pushname}`,
previewType: "PHOTO",
 thumbnailUrl: photo,
...
const vid = { url : `wa.me/6281359053674`, type : "video/mp4" }         
...
m.reply(`${global.botName} Aktif ngaf`)
}
 /* ~~~~~~~~~ RESPON USER AFK ~~~~~~~~~ */
...
let yameteh = `Maaf ngaf @${sender.split('@')[0]} limit kamu sudah habis!`
ehz.sendMessage(from, {text: cuih},{quoted: m})
}

const onlyGlimit = async() => {
let kyahh = `Maaf ngaf @${sender.split('@')[0]} limit game kamu sudah habis!`
ehz.sendMessage(from, {text: cuih},{quoted: m})
}
const onlyOwner = async() => {
setReply(`Ngapain Sialan??, Fitur ini khusus Owner!`)
}
...
`: ''}
${isWin ? `*SEMUA JAWABAN TERJAWAB âœ…*` : isSurrender ? '*MENYERAH âŒ*' : ''}
                ...
                if (!m.key.fromMe && secreto.find(i => i.sender === sender)) {
                   ...
                   if (dbx.status === 'ENTER-MESSAGE') {
                     if (['conversation', 'extendedTextMessage'].includes(m.type)) {
                       var teks_balasan = `Hai ngaf, kamu menerima pesan balasan nih\n\nPesan Balasannya:\n${budy}\n\n\n_ingat pesan ini satu kali kirim saja yaa_`
...
case 'confes':
                case 'menfess': case 'menfes': {
                   ...
                   if (args.length < 1) return setReply (`*Cara penggunaan :*\n${prefix+command} nomer|nama pengirim|pesannya\n\n*Note :* nomer harus format internasional dan nama pengirim boleh nama samaran atau rahasia\n\n*Contoh :* ${prefix+command} ${botNumber.split("@")[0]}|Seseorang|Hai ngaf`)
                   var no = q.split('|')[0];
				 					 if (isNaN(no)) return setReply (`Tujuan Harus Berupa Nomor!\n\nContoh :\n${command} `+botNumber.split("@")[0]+'|Anonim|Hai ngaf')
				           var num = no.replace(/[^0-9]/g, ""); 
                   var name = q.split('|')[1]; 
                   var pesan = q.split('|')[2];
                    if (!num) return setReply (`Format salah, contoh pemakaian : ${prefix+command} ${botNumber.split("@")[0]}|Anonim|Hai ngaf`)
                   if (!name) return setReply (`Format salah, contoh pemakaian : ${prefix+command} ${botNumber.split("@")[0]}|Anonim|Hai ngaf`)
                   if (!pesan) return setReply (`Format salah, contoh pemakaian : ${prefix+command} ${botNumber.split("@")[0]}|Anonim|Hai ngaf`)
                  var num = num.replace(/[^0-9]/gi, '')+'@s.whatsapp.net'
                   ...
                   if (botNumber === num) return setReply (`Jangan kirim ke nomer bot ngafðŸ¥²`)
...
case 'balas':{
                    ...
                    if (isSecreto) {
                     if (isSecreto.status === 'ENTER-MESSAGE') {
                       m.reply(`Mau dibalas apa ngaf? Silahkan kirim pesan balasannya`)
                     }
                   } else {
                     var obj = {
                       sender: sender,
...
`+readmore+`
         *MAIN MENU*
â•­à­§â¤âƒâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ˆâ—¦â€¢â—¦â¥â€¢â—¦
â”‚â€¢âŸ¢.allmenu
â”‚â€¢âŸ¢.rules
â”‚â€¢âŸ¢.ping
â”‚â€¢âŸ¢.runtime
ê’°âš˜Ý„ê’±â‚Š___________________Ë“Ë“ â¥âƒâƒ Ë’Ë’
`
ehz.sendMessage(m.chat, {
    react: {
      text: 'ðŸ”',
...
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
if (!isGroup) return setReply(mess.only.group)
if (!q) return setReply(`Teks Nya Mana ngaf?`)
   ...
   if (!isGroup) return onlyGroup()
    ...
    break
            case 'afk':
                ...
                if (isAfkOn) return setReply("ngafak Sudah Afk Sebelumnya")
...
setReply(`Oke ngaf Reportmu Sudah Terkirim Ke Owner`)
...
case 'antilinkgc':{
	...
	if (isAntilinkGc) return setReply('Fitur sudah aktif ngaf')
...
if (!isGroupAdmins) return setReply(mess.Badmin)
if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
if (isAntiLink) return setReply('Fitur sudah aktif ngaf')
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `RangelÂ°á­„á´®áµ’áµ—`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/a20c924a5f7e7953607aa.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
...
if (!isGroup) return onlyGroup()
if (!isPremium && global.db.data.users[sender].limit < 1) return onlyLimit()
if (!isUrl) return setReply("Masukan link youtube nya ngaf")
...
sendSticker(stikot)
                ...
                ehz.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/upscale?apikey=${api.Lol}&img=${anu}` }, caption: "nih ngaf >\<" }, { quoted: m })}
            break
...
case "fetch":
 ...
 if (!isMedia) return setReply("Where The A Image")
                ...
                let anu = await TelegraPh(media)
                ehz.sendMessage(m.chat, {
                    image: { url: `https://api.lolhuman.xyz/api/removebg?apikey=${api.Lol}&img=${anu}` },caption: "nih ngaf>\<" }, { quoted: m}) }
            break
  case 'flaming1':{
if (!isGroup) return onlyGroup()
...
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/a20c924a5f7e7953607aa.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: m})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
const bisa =['BISA','Tidak Bisa','Oh tentu saja bisa dong','Udah dari lahir dia bisa kaya gitu ngaf ðŸ˜‚Ëœâ€ž','Oh tentu saja tidak bisa','Wuih bisa bisa','Ga mao jawab ah lu wibu','Tentu saja bisa eh tapi boong awokawok ','Engga engga dia ga bisa','Enggaklah','Aku ga mao jawbab ðŸ˜†â„¢â€š','Rahasia dong','Ulangi Tod gua ga paham','Mana gua tau anjir']
...
ehz.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${week} , ${calender}ã…¤ã…¤ã…¤ã…¤ã…¤ã…¤ã…¤ã…¤ã…¤`,body:`âŒœ ${ucapanWaktu} âŒŸ`,previewType:"PHOTO",thumbnail: its,sourceUrl: `wa.me/6281359053674`,}
...
const cakep =['Iyaa cakep','Maaf ngaf, BanyakÂ² Perawatan Ya','Jelek ajg','Cakep bangetðŸ˜','âŒ','âœ”ï¸']
...
ehz.sendMessage(from, { text : `Hai ngaf @${sender.split("@")[0]}, Hii itu nomor owner ku jan spam yak:)`, contextInfo:{
forwardingScore: 9999999, 
isForwarded: true,
mentionedJid:[sender]
}}, { quoted: repf })
...
let oah = ' Ada apa ngaf itu nomor ayang ku mmmm><'
setReply(oah)
break
case 'jam':
...
setReply(`*ã€Œ Pesanan Diproses ã€*\nPESANAN ANDA AKAN KAMI PROSES JADI DI MOHON UNTUK MENUNGGU SEBENTAR ngaf`)
...
//======== MENU OWNER ======//
 case 'ban':{
			...
			if(cekBannedUser (woke, ban)) return setReply("Udah di ban ngaf")
			...
			if(cekBannedUser (Nomer, ban)) return setReply("Udah di ban ngaf")
...
case 'addprem':
		...
		ehz.sendText(mentionByTag, `Halo ngaf aku *${fake}*\nkamu telah terdaftar sabagai user premium\nterimakasih sudah menggunakan ${fake}\n\nSilahkan ketik .cekprem untuk melihat expired premium kamu`)
		   } else if(!isGroup){
		let usernya = q.split("|")[0].replace(new RegExp("[()+-/ +/]", "gi"), "") + `@s.whatsapp.net`
		let waktunya = q.split("|")[1]
		
		// if(_prem.checkPremiumUser (usernya, premium)) return setReply("User tersebut sudah di premium ngaf")   
		_prem.addPremiumUser (usernya, waktunya, premium)         
		await setReply(`Succses, silakan ketik ${prefix}listprem untuk melihat list premium`)
		ehz.sendText(usernya, `Halo ngaf aku *${fake}*\nkamu telah terdaftar sabagai user premium\nterimakasih sudah menggunakan ${fake}\n\nSilahkan ketik .cekprem untuk melihat expired premium kamu`)
...
await ehz.sendMessage(from, { document: fs.readFileSync("./all/database/contacts.vcf"), fileName: "contacts.vcf", caption: "Nih ngaf Tinggal Pencet File Di Atas Terus Save", mimetype: "text/vcard", }, { quoted: m })
...
await ehz.sendMessage(from, { document: fs.readFileSync("./database/contacts.vcf"), fileName: "contacts.vcf", caption: "Sukses Tinggal Save Ya ngafak", mimetype: "text/vcard", }, { quoted: m })
contacts.splice(0, contacts.length)
fs.writeFileSync("./database/contacts.json", JSON.stringify(contacts))
}
}
...
let teks = ('ngapain Ksk Kirim Ljnk Group,Jika Ingin Masukin Bot Ke Group ngafa Chat Owner Ku Terlebih Dahulu Ya ngaf')      
m.reply (teks)
}
